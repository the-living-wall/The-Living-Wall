...
低延迟实验（#89）：合并前在独立副本检出 `livehighhigh/89-depth-latency`，合并后使用 main；网页、采集脚本、`frame_stream.py` 与服务必须来自同一提交。停止旧程序后重新启动并校准，不混用新旧管道协议。启用深度实验后可展开“输入诊断”，记录 60 秒并导出数值。协议错误须更新完整启动链路。预览最多 10Hz，原始绿色圆圈独立更新；不要把预览或光团缓动计为取点速度。用户已确认现场相机为 Gemini 335，与当前适配一致。
