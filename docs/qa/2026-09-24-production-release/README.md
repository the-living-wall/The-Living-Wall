...
Git 端点直连失败而 GitHub API 可用；只读检查确认系统代理已启用，但 Git 未采用代理。通过现有代理访问 github.com 在约1.9秒返回200，随后仅为本次命令指定该代理，fetch main 成功。未修改系统或全局 Git 设置，不再需要 Git 对象 API 替代方案。

桌面应用日志报 `No ChatGPT browser route is available`，对应当前任务；这将浏览器故障定位到任务与浏览器控制后端的会话路由。可见网页仍打开不代表自动控制路由可用，未发现新的网页登录/权限拒绝。为何路由丢失尚未查明，停止无变化重试；需要应用重建会话后再验收。

已正常合并 main b51e7af：package.json 保留完整腾讯构建和全部朋友测试；共享塑造 tasks 保留两边历史，最新验收说明在后。解决两处文本冲突后，完整 CI 重新验证。
