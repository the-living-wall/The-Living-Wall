...
普通启动仍为 `.venv/bin/python server.py --port 8769`，USB 权限失败时使用本目录 `启动相机权限测试.command`；前端在项目根目录运行 `npm run dev -- --port 3018`。`frame_stream.py` 必须和两个 Python 入口放在同一目录。用户已确认现场设备为 Gemini 335，当前代码的型号选择与设备一致；连接时记录 SDK 枚举信息和流配置。
