...
接好 Gemini 335 和投影仪后的当前操作顺序，见 [Mac 真实硬件启动指南](../../docs/testing/depth-hardware-startup.md)（2026-09-26 核对）。其中区分首次安装/日常启动，说明 3018 前端与 8769 深度服务、空墙校准、设备移动后的重校准和断线恢复；主前端联调以该指南为准。
...
## macOS USB 拒绝访问（uvc_open -3，历史实测记录）

2026-09-15 设备已以 USB 3 识别，但普通启动（包括用户终端）无法打开接口。若出现此错误，可双击 `启动相机权限测试.command`，在系统终端内输入 Mac 管理员密码。脚本只提升 `camera_capture.py` 相机读取进程；通过匿名管道把深度帧交给普通权限的网页服务（当时为 127.0.0.1:8767；当前脚本为 127.0.0.1:8769）。不修改系统长期权限、不执行固件更新，不在聊天中索要密码。关闭启动窗口或 Ctrl+C 结束会话。
