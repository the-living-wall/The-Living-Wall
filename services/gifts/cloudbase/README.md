...
Refs #88。目标是用户新建的 `the-living-wall` 独立个人版环境 `env-d1g2bv5sn355fc36e`，自带共享文档实例 `tnt-4tn13ikbi`。不得操作 `ai-future-letter`。应用源提交 `7e30d0b` 已于 2026-09-23 部署到独立测试来源，尚未完成全部验收；[测试步骤、证据与未完成项](../../../docs/qa/2026-09-23-cross-device/cloud-first-deploy/README.md)。下文配置说明不能代替该部署记录。

## 已准备与未验证

2026-09-23 现有主站的可选 SDK 接入已完成本地实现；云端权限未开放，新增函数未部署。具体差异与回退见 [SDK 最小验证](SDK-VALIDATION.md)。下述原 HTTP 部署流程保持独立，不能把两种入口的权限混用。

已实现服务端事务、同版本双方确认、幂等请求、独立访问凭证、七天有效期及定时清理适配；本地契约测试模拟 SDK 事务内对象/事务外数组的返回结构及乐观冲突。本地契约测试不证明部署；真实云端三场景、并发与幂等补验见上述记录。
