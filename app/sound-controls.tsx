...
export default function SoundControls({
  ...
  const [soundOn, setSoundOn] = useState(false),
    ...
    [musicReady, setMusicReady] = useState(false),
  ...
  const toggleSound = () => {
    ...
    if (loading || (!effectsReady && !musicReady)) {
      enabledRef.current = true;
      void activate();
      return;
    }
  ...
  const anyReady = effectsReady || musicReady;
  const partial = !loading && effectsReady !== musicReady;
  ...
  return (
    <div className="sound-controls">
      <div className="sound-row">
        <Button onClick={toggleSound} aria-pressed={anyReady}>
          {loading ? '声音准备中…' : anyReady ? '关闭声音' : '开启声音'}
        </Button>
      ...
      {statusMessage && (
        <output className="sound-message">{statusMessage}</output>
      )}
      {partial && (
        <button
          className="sound-retry"
          type="button"
